...
class RecommendationsTestCase(unittest.TestCase):
    ...
    def test_user_get_difference(self):
        user1 = User(1, [0, 1, 2, 2, 3])
        user2 = User(2, [4, 5, 5, 6])
        user3 = User(3, [2, 3, 4, 5, 6])

        self.assertEqual(user2.get_difference(user3.unique_views), set([]))
        self.assertEqual(user1.get_difference(user3.unique_views), {0, 1})
        self.assertEqual(user3.get_difference(user1.unique_views), {4, 5, 6})
